        ...
        for dialogUtterance in utterances:
            ...
            if dialogUtterance.pos:
                for posTag in dialogUtterance.pos:
                    if posTag.token in vocab_dict:
                        feature.append(vocab_dict[posTag.token])
                    else:
                        feature.append(vocab_dict["<UNCH>"])
                for posTag in dialogUtterance.pos:
                    if posTag.token in pos_dict:
                        feature.append(pos_dict[posTag.pos])
                    else:
                        feature.append(pos_dict["<UNCH>"])
            file.append(feature)
        xTrain.append(file)
...
for utterances in files_train:
    for dialogUtterance in utterances:
        if dialogUtterance.pos:
            for posTag in dialogUtterance.pos:
                if not posTag.token in vocab_dict:
                    vocab_dict[posTag.token] = len(vocab_dict)
                if not posTag.pos in pos_dict:
                    pos_dict[posTag.pos] = len(pos_dict)
vocab_dict["<UNCH>"] = len(vocab_dict)
pos_dict["<UNCH>"] = len(pos_dict)
...
trainer.params()
trainer.train('baseline_model_new.crfsuite')
